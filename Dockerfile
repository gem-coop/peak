...
RUN --mount=type=secret,id=BUNDLE_PACKAGER__DEV \
    bundle config set https://packager.dev/avo-hq/ "$(cat /run/secrets/BUNDLE_PACKAGER__DEV)" && \
    bundle install && \
...
# Make bun put application.js into app/assets/
RUN bun run build
