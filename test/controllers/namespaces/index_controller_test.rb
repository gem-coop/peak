...
class Namespaces::IndexControllerTest < ActionDispatch::IntegrationTest
  ...
  test "get index" do
    ...
    assert_match /^oaken/, response.body
  end
  ...
  test "get show with dependencies" do
    ...
    assert_includes response.body.split("\n"),
      "8.1.1 activemodel:= 8.1.1,activesupport:= 8.1.1,timeout:>= 0"

    get namespaces_info_url(namespace: namespaces.public, id: gems.activesupport)
    assert_response :success
    assert_includes response.body.split("\n"),
      "8.1.1 base64:>= 0,bigdecimal:>= 0,concurrent-ruby:>= 1.3.1&~> 1.0,connection_pool:>= 2.2.5,drb:>= 0,i18n:< 2&>= 1.6,json:>= 0,logger:>= 0"
  end
end
